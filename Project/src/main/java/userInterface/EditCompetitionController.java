...
public class EditCompetitionController implements Initializable{
    ...
    void editCompetition(ActionEvent event) throws IOException {
		...
		try {
	    	...
	    	CommonMethods.closeWindow(event);
	    	int i;
	    	for (i = 0; i < Start.competitionManager.getCompetitions().size(); i++) {
	    		if (Start.competitionManager.getCompetitions().get(i).getName().equals(tfEditCompetitionName.getPromptText())) {
	    			Competition competition = Start.competitionManager.getCompetitions().get(i);
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		dpEditCompetitionDueDate.setDayCellFactory(CommonMethods.disablePreviousDates);
	}
