...
public class MainMenuController implements Initializable {
    ...
    void showAddParticipantWindow(ActionEvent event) throws IOException {
    	...
    	Competition competition = Start.competitionManager.getCompetitions().get(competitionIndex);
    ...
    void showCompetitionTabs() throws IOException {
    	for (int i = 0; i < Start.competitionManager.getCompetitions().size(); i++) {
    		if (Start.competitionManager.getCompetitions().get(i) instanceof IndividualBasedCompetition) {
	        	...
	        	Tab tab = new Tab(Start.competitionManager.getCompetitions().get(i).getName(), root);
    ...
    void deleteCompetition(int competitionIndex) throws IOException {
		...
		controller.competition = Start.competitionManager.getCompetitions().get(competitionIndex);
