...
public class CompetitionTabTeamController {
    ...
    @FXML
    private TableColumn<Participant, Integer> tcTeamNumber;
    
    void competitionTab(int index) {
    	Competition competition = (TeamBasedCompetition) Start.competitionManager.getCompetitions().get(index);
    	...
    	tcTeamNumber.setCellValueFactory(new PropertyValueFactory<>("TeamNumber"));
    ...
    void editParticipant(ActionEvent event) throws IOException {
		...
		FXMLLoader loader = new FXMLLoader(getClass().getResource("Edit_ParticipantTeam.fxml"));
		Parent root = loader.load();
		EditParticipantTeamController controller = loader.getController();
		controller.prepareTextfields(participant.getName(), participant.getId(), participant.getMajor(), participant.getTeam().toString(), participant.getRank(), competition.getName());
    ...
    void prepareEmail(ActionEvent event) throws IOException {
		...
		int i;
		for (i = 0; i < team.getParticipants().size()-1; i++) {
			mailTo += "s" + team.getParticipants().get(i).getId() + "@kfupm.edu.sa,";
		}
		mailTo += "s" + team.getParticipants().get(i).getId() + "@kfupm.edu.sa?";
		String subject = "subject=Congratulation on achieving " + participant.getRank() + " place in " + competition.getName()+"&";
		String body = "body=Dear " + team.getName() + ",\n"
				+ "\n"
